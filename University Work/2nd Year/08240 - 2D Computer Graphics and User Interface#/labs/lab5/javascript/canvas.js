...
function onLoad()
{
	...
	{ 
		...
		{ 
			case 37:
				car.setCarSpeed(new Vector(-20, 0));
				break;
			case 39:
				car.setCarSpeed(new Vector(20, 0));
				break;
	...
	function initialise ()
	{
		...
		car = new Car(new Vector(-1000, 310), new Vector(20, 0), '#FFC0CB', 1, 0, 20);
	...
	function gameLoop()
	{
		update();
		draw();
		//setTimeout(gameLoop, 1000/60); // setTimeout Method
		requestAnimationFrame(gameLoop);
	}
		
	// call the initialise and draw functions
	//setInterval(gameLoop, 1000/60); // setInterval method
	initialise();
	setupEventHandlers();
	gameLoop(); // setTimeout method / webkitRequestAnimationFrame
